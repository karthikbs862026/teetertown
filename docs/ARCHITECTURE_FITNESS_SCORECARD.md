...
| Level unload returns resource baseline                   | No monotonic registry growth after 50 post-warm-up cycles      | Simulation `11/11/2` ×50; CI browsers stable across 20 transitions | Partial pass; memory/soak pending |
...
| Test feedback remains small-team practical               | Fast checks ≤2 min local; browser checks separated             | Headless suite ≈2 s; engine-isolated CI tests 35–57 s each         | Pass                              |
...
Gate recommendation cannot exceed **ITERATE** until physical-device, representative-player,
performance/soak, offline/update, and remaining adversarial-fixture rows have evidence.
