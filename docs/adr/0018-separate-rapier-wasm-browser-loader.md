...
- Bootstrap: `74e1d58f` in Node and actual Chromium.
- Golden: success at step 278, state hash `c965c01f` in Node and actual Chromium.
...
collapsed into the Rapier semantic version. Any unexplained hash difference blocks release. Firefox,
WebKit, physical Android/iOS, offline/service-worker WASM caching, and future Rapier upgrades remain
required evidence. Revert this decision if same-origin/offline loading is brittle, the runtime
variants diverge, or payload interaction timing misses the measured startup budget.
