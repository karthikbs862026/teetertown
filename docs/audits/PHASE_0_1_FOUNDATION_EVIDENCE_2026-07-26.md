...
- Stable-ID data schemas, content/material hashes, primitive colliders, visible hinges, bounds, goal
  hold/speed rule, accumulated fragility window, and classified terminal failures.
...
- Versioned IndexedDB boundary, tested memory adapter, no-op Phase 1 platform services, PWA
  manifest, service-worker lifecycle shell, and an empty provenance/asset manifest.
...
- Headless physics: 4,000 steps, p50 0.0918 ms, p95 0.2494 ms, p99 0.4632 ms, maximum 9.4454 ms on
  Node v24.14.0. These numbers do not establish a frame-rate tier.
